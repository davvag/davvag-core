(function (w){
    ...
    function ComponentInstance(appId,compId, descriptor, appdescriptor){
        ...
        var exports =  {
            ...
            getAppComponent: function(compAppId,c, cb){
                ...
                if (canDownload){
                    componentManager.downloadAppDescriptor(compAppId, function(descriptor){
                        var componentVersion = descriptor && descriptor.description ? descriptor.description.version : undefined;
            ...
            ensureBackendHandlers: function(nextDescriptor){
                if (nextDescriptor)
                    descriptor = nextDescriptor;
                attachBackendHandlers(descriptor);
                return this;
            },
            componentId: compId
        }
        ...
        function attachBackendHandlers(sourceDescriptor){
            if (!sourceDescriptor)
                return;

            if (sourceDescriptor.serviceHandler){
                var mObj = sourceDescriptor.serviceHandler.methods; 
                if (mObj)
                injectBackendFunction("services","service", mObj);
            }

            if (sourceDescriptor.transformers){
                var mObj = sourceDescriptor.transformers; 
    ...
    function AjaxRequestor(url, postParams, method){
        ...
        function isNetworkError(data, textStatus){
        ...
        function errorFunc(data, textStatus, errorThrown){
            if (retries < maxNetworkRetries && isNetworkError(data, textStatus)){
                retries++;
                issueRequest();
                return;
            }
    ...
    var componentManager = (function (){
        ...
        function isServiceComponent(appdescriptor, compId){
            if (!appdescriptor || !appdescriptor.components || !appdescriptor.components[compId])
                return false;

            var component = appdescriptor.components[compId];
            return component.type && component.type.toString().toLowerCase() === "service";
        }

        function hasUsableCachedInstance(appdescriptor, compId, instance){
            if (!instance)
                return false;

            if (isServiceComponent(appdescriptor, compId) && !instance.services)
                return false;
                ...
                switch(obj.type){
                    case "mainScript":
                        ...
                        if (!settings.componentRepo[obj.appId][obj.component])
                            ...
                            if (initializingPlugin.ensureBackendHandlers)
                                initializingPlugin.ensureBackendHandlers(obj.descriptor);
                        }
                        var scriptInjector = new ScriptInjector();
                        scriptInjector.onScriptLoaded(function(){
        ...
        function getOnDemand(appId,appdescriptor,compId,cb,verid){
            ...
            if(hasUsableCachedInstance(appdescriptor, compId, window["getOnDemandApp"][appId][compId].instance)){
