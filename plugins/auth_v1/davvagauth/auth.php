...
class AuthSvr {
    public static function Login ($username, $password){
        $loginData=self::getObjectForGetMethod(AUTH_URL . "/login/$username/$password/".AUTH_DOMAIN);
    ...
    public static function SocialLogin ($app, $code,$create){
        return self::getObjectForGetMethod(AUTH_URL . "/social/$app/$code/$create");
    }

    public static function GetResetToken ($email){
        return self::getObjectForGetMethod(AUTH_URL . "/getresettoken/$email/123");
    }
	
	public static function SaveUser ($user){
        return json_decode(self::callRest(AUTH_URL . "/createuser/","POST",$user));
    }
    ...
    public static function NewDomain ($data){
        ...
        if($result->success){
            ...
            $d=json_decode(self::callRest(AUTH_URL . "/newdomain/","POST",$data));
            //var_dump($d);
    ...
    public static function Join ($domain,$userid,$usergroup){
        return self::getObjectForGetMethod(AUTH_URL . "/join/$domain/$userid/$usergroup");
    }

    public static function ResetPassword ($email, $token, $newPassword){       
        return self::getObjectForGetMethod(AUTH_URL . "/resetpassword/$email/$token/$newPassword");
    }


    public static function ChangePassword ($oldpassword, $newPassword){       
        return self::getObjectForGetMethod(AUTH_URL . "/resetpassworduser/$oldpassword/$newPassword");
    }

    public static function GetSession ($token){
        return self::getObjectForGetMethod(AUTH_URL . "/getsession/$token");
    }

    public static function GetLogout ($token){
        $outObject=self::getObjectForGetMethod(AUTH_URL . "/logout/$token");
        //$outObject = self::GetLogout($authdata->token);
    ...
    public static function GetUserGroups (){
        return self::getObjectForGetMethod(AUTH_URL . "/usergroups/");
    }

    public static function NewUserGroup ($groupid){
        return self::getObjectForGetMethod(AUTH_URL . "/newusergroup/$groupid");
    }
    ...
    public static function GetAccess ($groupid,$app,$type=null,$code=null,$ops=null){
        ...
        if(isset($type) && isset($code) && isset($ops)){
            ...
            if(isset($result)){
                ...
                $r=self::getObjectForGetMethod(AUTH_URL . "/getacess/$groupid/$app/$type/$code/$ops/");
                if(!isset($r->error)){
            ...
            if(isset($result)){
                ...
                $r=self::getObjectForGetMethod(AUTH_URL . "/getacess/$groupid/$app/");
                if(!isset($r->error)){
                    CacheData::setObjects($token,"sys_access",$r);
                }
                return $r;
    ...
    public static function SetAccess ($uapp){
        return json_decode(self::callRest(AUTH_URL . "/setaccess/","POST",$uapp));
    }

    public static function GetDomainAttributes (){
        return self::getObjectForGetMethod(AUTH_URL . "/getdomain/".AUTH_DOMAIN);
    }



    private static function getObjectForGetMethod($url){
        $output = self::callRest($url);
    ...
    public static function CrossDomainAPICall($domain,$url,$method="GET",$data=null){
        ...
        return json_decode(self::callRest($url,$method,$data,array("Content-Type: application/json", "rhost:".AUTH_DOMAIN,"sosskey: $securityToken")));
    }

    public static function AutendicateDomain($tname,$securityToken,$appname,$operation){
        $user =self::GetSession($securityToken);
